...
#  }
  ...
  if (family$family == "Gamma") {
    ...
    # Diagnostics
    #avg_cpp_draws <- mean(cpp_draws)
    #  cat(sprintf("C++ sampler average draws: %.3f\n", avg_cpp_draws))
...
rGammaGamma <- function(
  ...
  # Validate output
  if (!is.list(tmp) || is.null(tmp$dispersion) || is.null(tmp$draws)) {
    stop("C++ rGammaGamma_cpp_export returned an invalid structure.")
  }
  
  cpp_disp  <- tmp$dispersion
  cpp_draws <- tmp$draws
...
#  cat(sprintf("C++ sampler average draws: %.3f\n", avg_cpp_draws))
  ...
  # ## ----------------------------------------------------------------------
  # ## Compute the mean vector μ_i = exp(α + xβ)
  # ## ----------------------------------------------------------------------
  # ## This is the fixed linear predictor under the Gamma GLM with log link.
  # ## The sampler treats β as fixed and samples only the precision v = 1/φ.
  # ## ----------------------------------------------------------------------
  ...
  # 
  # ## ----------------------------------------------------------------------
  # ## 1. Original log-likelihood for precision v under the Gamma GLM
  # ##
  # ##     ll(v) = Σ_i w_i [ v log v − v log μ_i + (v−1) log y_i
  # ##                       − log Γ(v) − v y_i / μ_i ]
  # ##
  # ## This is concave in v for the Gamma GLM with log link.
  # ## ----------------------------------------------------------------------
  # loglik <- function(v) {
  #   sum(
  #     wt * (
  ...
  # 
  # ## ----------------------------------------------------------------------
  # ## 2. Proxy log-likelihood ll*(v)
  # ##
  # ##     ll*(v) = ll(v) − (1/2) Σ_i w_i log v
  # ##
  # ## This adjustment preserves concavity and yields a cleaner envelope.
  # ## ----------------------------------------------------------------------
  # loglik_star <- function(v) {
  #   loglik(v) - 0.5 * W_sum * log(v)
  # }
  # 
  # ## ----------------------------------------------------------------------
  # ## 3. rate1: deviance-based surrogate rate used in the fixed-point update
  # ##
  # ##     rate1 = rate_prior + Σ_i w_i [ y_i/μ_i − log(y_i/μ_i) − 1 ]
  # ##
  # ## This is used to locate an approximate posterior mode.
  # ## ----------------------------------------------------------------------
  # rate1 <- rate + sum(wt * ((y / mu1) - log(y / mu1) - 1))
  # 
  # ## ----------------------------------------------------------------------
  # ## 4. Posterior mode finder via Gamma-surrogate fixed-point iteration
  # ##
  # ## Start with:
  # ##     v_star^(0) = shape_prior / rate1
  # ##
  # ## Then iterate:
  # ##     v_{new} = v_star^(0) − (v / rate1) Σ_i w_i [ ψ(w_i v) − log(w_i v) ]
  # ##
  # ## This converges rapidly for the Gamma GLM.
  # ## ----------------------------------------------------------------------
  # shape2_mode <- shape
  # vstar1      <- shape2_mode / rate1
  # 
  ...
  # 
  # ## ----------------------------------------------------------------------
  # ## 5. Exact second derivative f''(v) of the log-posterior
  # ##
  # ##     f(v) = ll(v) + (shape−1) log v − rate v
  # ##
  # ## Used to build a Gamma approximation to the posterior.
  # ## ----------------------------------------------------------------------
  # f2_exact <- function(v) {
  #   term1 <- W_sum * (1 / v)          # from log v terms
  #   term2 <- -W_sum * trigamma(v)     # from −log Γ(v)
  #   term3 <- -(shape - 1) / (v^2)     # prior curvature
  #   term1 + term2 + term3
  # }
  # 
  ...
  # 
  # ## ----------------------------------------------------------------------
  # ## 6. Gamma surrogate posterior for v:
  # ##
  # ##     v | y ≈ Gamma(ᾱ, β̄)
  # ##
  # ## where:
  ...
  # ##     β̄ = −f''(v*)
  # ##
  # ## This gives a good approximation to the posterior mean.
  # ## ----------------------------------------------------------------------
  # alpha_bar <- 1 - f2_vstar * vstar^2
  # beta_bar  <- -f2_vstar * vstar
  # 
  ...
  # 
  # ## ----------------------------------------------------------------------
  # ## 7. Compute surrogate-based bounds for v
  # ##
  # ##     v_min = qgamma(1 − p, ᾱ, β̄)
  # ##     v_max = qgamma(p,     ᾱ, β̄)
  # ##
  # ## These define a high-probability region for v.
  # ## ----------------------------------------------------------------------
  # v_min <- qgamma(1 - max_disp_perc, shape = alpha_bar, rate = beta_bar)
  # v_max <- qgamma(max_disp_perc,     shape = alpha_bar, rate = beta_bar)
  # 
  # ## ----------------------------------------------------------------------
  # ## 8. Convert user-supplied dispersion bounds to precision bounds
  # ##
  # ## If user did not supply bounds, use surrogate-based defaults.
  # ## ----------------------------------------------------------------------
  ...
  # 
  # ## ----------------------------------------------------------------------
  # ## 9. Tangency point for envelope construction
  # ##
  # ## We use v_tangent = posterior mean (A6-optimal anchor).
  # ## ----------------------------------------------------------------------
  # v_tangent  <- v_mean
  # ll_star_v0 <- loglik_star(v_tangent)
  # ll_v0      <- loglik(v_tangent)
  # 
  # ## ----------------------------------------------------------------------
  # ## 10. Gradient of −ll*(v) at the tangency point
  # ##
  # ##     ll'(v) = Σ_i w_i [ log v + 1 − log μ_i + log y_i
  # ##                        − ψ(v) − y_i/μ_i ]
  # ##
  # ##     ll*'(v) = ll'(v) − (1/2) Σ_i w_i (1/v)
  # ##
  # ## We temporarily omit the adjustment term for stability.
  # ## ----------------------------------------------------------------------
  # ell_prime_at <- function(v) {
  #   sum(
  #     wt * (
  ...
  # }
  # 
  # ## cbar = gradient of −ll*(v) at v_tangent
  # cbar <- -ell_star_prime_at(v_tangent)
  # 
  # ## ----------------------------------------------------------------------
  # ## 11. Proposal Gamma(shape_prop, rate_prop)
  # ##
  # ##     shape_prop = shape + (1/2) Σ_i w_i
  # ##     rate_prop  = rate − cbar
  # ##
  # ## This proposal is centered near the posterior mean.
  # ## ----------------------------------------------------------------------
  # shape_prop <- shape + 0.5 * W_sum
  # rate_prop  <- rate - cbar
  # 
  ...
  # 
  # ## ----------------------------------------------------------------------
  # ## 12. Bounding function log h(v)
  # ##
  # ##     log h(v) = ll*(v) − [ ll*(v0) − cbar (v − v0) ] − (1/2) Σ_i w_i log(v/v_min)
  # ##
  # ## This ensures log h(v) ≤ 0 for all v in the domain.
  # ## ----------------------------------------------------------------------
  # lg_h <- function(v) {
  #   loglik_star(v) -
  ...
  # 
  # ## ----------------------------------------------------------------------
  # ## 13. Rejection sampling for precision v
  # ##
  # ## For each draw:
  # ##   (a) sample v_candidate from proposal Gamma(shape_prop, rate_prop)
  # ##   (b) accept if log h(v_candidate) − log U > 0
  # ##
  # ## Convert accepted v to dispersion φ = 1/v.
  # ## ----------------------------------------------------------------------
  # out   <- numeric(n)
  # test  <- numeric(n)
  # a     <- numeric(n)
  ...
  #   while (a[i] == 0) {
  #     
  #     ## Draw from truncated or untruncated proposal
  #     if (is.null(disp_lower) && is.null(disp_upper)) {
  #       cand_prec <- rgamma_ct(
  #         1,
  #         shape = shape_prop,
  ...
  # out <- 1 / out
  # 
  # # # Diagnostic: average draws from the R implementation
  # # if (!is.null(draws)) {
  # #   avg_r_draws <- mean(draws)
  # #   cat(sprintf("R sampler average draws: %.3f\n", avg_r_draws))
  # # }
  # 
  # list(
  #   dispersion = out,
