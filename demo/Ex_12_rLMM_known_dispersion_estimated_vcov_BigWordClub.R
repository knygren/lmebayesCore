## Demo: rLMMNormal_reg_estimated_vcov() called directly on bayesrules::big_word_club
##
## Case 3 of 5: KNOWN observation dispersion (sigma^2 fixed at its lmer REML
## estimate), ESTIMATED random-effect variance components (every Block~2
## component is dIndependent_Normal_Gamma() -- tau^2_k is sampled, not
## fixed). Two-block Gibbs with a pilot stage (Theorem~3 rate calibration).
##
...
##
##   demo("Ex_12_rLMM_known_dispersion_estimated_vcov_BigWordClub", package = "lmebayesCore")
...
##    by hand so rLMMNormal_reg_estimated_vcov() can be called directly.
## ---------------------------------------------------------------------------
...
## ---------------------------------------------------------------------------
## 4. Direct call: rLMMNormal_reg_estimated_vcov()
##
## Two-block Gibbs is the only engine here (tau^2_k is estimated, so the
## joint posterior is not exactly Gaussian): an optional pilot stage
...
## per stored draw.
##
## progbar/verbose match demo("Ex_21_lmerb_ING_BigWordClub", package =
...
## sampled tau^2_k (fit$popef.dispersion[i, ]) and its own beta_j/gamma_p
## (fit$groupef/fit$popef draw i) -- and hence its own u_jp. Treating
...
## ---------------------------------------------------------------------------
## 5. tau^2_k: post-sweep draws stayed inside the calibrated Gamma window
## ---------------------------------------------------------------------------
...
## of an iid engine, same column layout as Ex_10/Ex_11's Section 5/6 (just
## without the 'iid' columns those have). 'diff(SE)' re-expresses the gibbs-
...
cat(
  "\n=== Block 2 fixed effects: Gibbs vs ICM mean vs lmer fixef (+ uncertainty) ===\n\n",
  ...
  rows_fe,
  "\n  diff(SE) = (gibbs mean - lmer estimate) / lmer Std. Error -- |diff(SE)| < ~1-2\n",
...
## for each stage (fit$pilot$chisq$n_pilot vs fit$n -- these can differ, e.g.
...
## started from (a genuine point estimate for this route -- unlike
## rLMMNormal_reg_known_vcov()'s sim_method = "DEFAULT" iid route, it does
## not depend on which draw happens to be last). This section only compares
## the sampler to itself (MCMC mean vs. its own ICM mode); it does not
## compare against lme4::coef(), since -- for RE components whose hyper-
## design W_k has covariates beyond an intercept ("(Intercept)" and
## "distracted_a1" here) -- coef()[[k]] alone omits the matching
## covariate/interaction contribution and is not beta_j (see Ex_10's
## Section 7 for the build_mu_all()-based reconstruction needed to make
