...
Many likelihoods used in generalized linear models belong to the **exponential family**, which can be written in the general form
...
- \( c(y,\phi) \) collects terms not involving \( \theta \)

For a GLM with linear predictor \( \eta = X\beta \) and mean \( \mu = b'(\theta) \), the log‑likelihood for \( n \) independent observations is
...
1. **For fixed dispersion \( \phi \)**,  
   ...
   \]
   and the cumulant function \( b(\theta) \) is convex.

2. **For fixed coefficients \( \beta \)**,  
   the log‑likelihood is often **concave in the precision**  
   \[
   \tau = \frac{1}{\phi},
   \]
   because \( a(\phi) \) typically enters as a positive scalar factor.
...
The Gaussian model is a special case where these properties become fully explicit.

---

## 2.2 Special Case: Gaussian Linear Regression

A Gaussian linear model with response vector \( y \in \mathbb{R}^{n} \), design matrix \( X \), coefficients \( \beta \), and dispersion \( \phi \) has density

\[
f(y \mid \beta,\phi)
= \exp\!\left(
  -\frac{1}{2\phi}(y - X\beta)^{T}(y - X\beta)
  - \frac{n}{2}\log(2\pi\phi)
\right).
\]

This fits the exponential‑family template with
...
### Log‑Concavity Properties in the Gaussian Case

1. **Concavity in \( \beta \)**  
   For fixed \( \phi \), the negative log‑likelihood is quadratic in \( \beta \):
   \[
   -\ell(\beta \mid \phi)
   = \frac{1}{2\phi}(y - X\beta)^{T}(y - X\beta) + \text{const},
   \]
   which is strictly convex.  
   Therefore the log‑likelihood is strictly concave in \( \beta \).

2. **Concavity in precision \( \tau = 1/\phi \)**  
   Rewriting the likelihood in terms of \( \tau \):
   \[
   \ell(\beta,\tau)
   = \frac{\tau}{2}(y - X\beta)^{T}(y - X\beta)
     - \frac{n}{2}\log\tau + \text{const},
   \]
...
The Gaussian model thus provides the cleanest illustration of the general exponential‑family structure and motivates the dispersion‑sampling strategies used throughout this chapter.
