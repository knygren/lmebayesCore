...
---
title: "Chapter 15: Tailoring Priors - Leveraging the Prior_Setup Function"
author: "Kjell Nygren"
date: "`r Sys.Date()`"
output: 
  rmarkdown::html_vignette:
    dfprint:: default
vignette: >
  %\VignetteIndexEntry{Chapter 15: Tailoring Priors - Leveraging the Prior_Setup Function}
...
This section compares posterior summaries for the height effect across five Bayesian prior configurations and a classical linear model. We report the posterior mean, the one-sided tail probability \(\Pr(\beta > \mu)\), the posterior standard deviation, and the effective number of parameters. These metrics reveal how prior centering and prior weight (pwt) influence inference, uncertainty, and model complexity.
...
directional_half_space_tail <- function(fit,
  ...
  post        <- fit$coefficients            # S x p matrix
  mu_prior    <- as.numeric(fit$Prior$mean)  # length-p
  V0          <- as.matrix(fit$Prior$Variance)  # p x p

  # 2. Compute prior precision and its symmetric square root
  P0    <- solve(V0)   # prior precision = Sigma_prior^{-1}
  ...
  #    Z = Sigma_prior^{-1/2} * (post - mu_prior)
  centered <- sweep(post, 2, mu_prior, FUN = "-")   # S x p
  Z        <- t(P0_sqrt %*% t(centered))            # S x p
  ...
  proj      <- Z %*% w         # S x 1 vector of t(w) %*% Z
  tail_prob <- mean(proj < 0)  # P(t(w) %*% Z < 0)
...
This section compares posterior summaries for the group effect (groupTrt) across five Bayesian prior configurations and a classical linear model. We report the posterior mean, the one-sided tail probability \(\Pr(\beta < 0)\), the posterior standard deviation, and the effective number of parameters. These metrics reveal how prior centering and prior weight (pwt) influence inference, uncertainty, and model complexity.
...
**Posterior Means**
- Changing `intercept_source` alone (Default vs. Null) shifts the intercept but leaves the slope nearly unchanged (-0.3486 to -0.3483).
...
- Increasing pwt (High-pwt) shrinks the slope toward zero, reducing its magnitude.

**Tail Probabilities \(\Pr(\beta < 0)\)**
- Weak or default priors (Default, Null, Low) yield \(\Pr(\beta < 0) \approx 0.11\text{--}0.13\), echoing the classical one-sided p-value.
- Full-model centering produces a symmetric posterior around the MLE, yielding \(\Pr(\beta < 0) \approx 0.48\).
- A strong skeptical prior (High-pwt) reduces certainty, lowering \(\Pr(\beta < 0)\) to 0.193.

**Effective Number of Parameters**
- Low-pwt behaves like an unregularized fit: Eff.Params \(\approx 1.00\).
- Default/Null priors (pwt = 0.05) modestly regularize: Eff.Params \(\approx 0.88\text{--}0.89\).
- Full-Model Prior (centered at the MLE) avoids shrinkage: Eff.Params \(\approx 0.96\).
- High-pwt (strong shrinkage) reduces flexibility: Eff.Params \(\approx 0.49\).

**Posterior Standard Deviations**
- As pwt decreases, Post.SD increases toward classical SEs (Low-pwt: 0.3165 vs. SE = 0.311).
- Default/Null priors yield modest SD reduction (0.2953 to 0.2992).
- Full-Model centering slightly inflates SD (0.3042).
- High-pwt sharply reduces SD (0.2223), reflecting equal prior--data precision.

In practice, selecting a prior involves balancing skepticism (via pwt) with anchoring assumptions (via `intercept_source` and `effects_source`). This comparison illustrates how those choices shape posterior estimates, uncertainty, and model complexity, offering a transparent framework for prior sensitivity analysis in Bayesian GLMs.
