...
Many common statistical models belong to the *exponential family*, a class of distributions that can be written in the form
...
- `theta` is the **canonical (natural) parameter**  
- `b(theta)` is the **cumulant function**, ensuring normalization  
- `a(phi)` is a **dispersion** or **scale** parameter  
- `c(y, phi)` collects terms not involving `theta`
...
- The mean is given by `mu = b'(theta)`.  
- The variance is `Var(Y) = b''(theta) * a(phi)`.  
...
When the link function is chosen so that `theta = eta`, the link is called **canonical**.  
...
- the link function g(μ)  
- the inverse link g⁻¹(η)  
...
- check that μ and η lie in valid domains  
...
The link function g(μ) = η determines how the mean of the response relates to the linear predictor.  
Each family has a canonical link, which ensures that:

- the canonical parameter equals the linear predictor (θ = η)  
- the log‑likelihood is often concave in β  
...
- The formula defines the linear predictor η = Xβ.  
- The family object provides the variance function V(μ) and the exponential‑family functions b(θ), b′(θ), and b″(θ).  
- The link provides g(μ), g⁻¹(η), and the derivative of the inverse link.  
- The IRLS algorithm solves the likelihood equations using the curvature implied by V(μ).  
...
1. **Formula**  
   ...
   tells glm how to build the model matrix X, and therefore the linear predictor η = Xβ.

2. **Family**  
   `poisson(...)`  
   constructs a family object describing the Poisson distribution and its variance function V(μ) = μ.
...
- Canonical links ensure that the canonical parameter `theta` equals the linear predictor `eta`, which often yields concave log‑likelihoods and stable IRLS updates.
