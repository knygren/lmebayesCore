...
---
title: "Chapter A2: Overview of Estimation Procedures"
author: "Kjell Nygren"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Chapter A2: Overview of Estimation Procedures}
...
envelope methods developed in Nygren and Nygren (2006). The envelope-based methods are documented
in detail in Appendix A2 and A5 (coefficient envelopes), Appendix A6 (Gamma-family dispersion envelopes),
and Appendix A7 (joint envelopes for Gaussian regression with independent Normal-Gamma priors).
...
- dNormal_Gamma selects rNormal_Gamma_reg
- dIndependent_Normal_Gamma selects rindependent_norm_gamma_reg
...
+----------------------+----------------------+------------------------------+------------------------------+---------------------------+
| Gaussian             | identity             | dNormal_Gamma                | rNormal_Gamma_reg            | Conjugate Normal-Gamma [R]|
+----------------------+----------------------+------------------------------+------------------------------+---------------------------+
| Gaussian             | identity             | dIndependent                 | rindependent                 | Envelope-based joint      |
|                      |                      | _Normal_Gamma                | _norm_gamma_reg              | sampler [C++]             |
+----------------------+----------------------+------------------------------+------------------------------+---------------------------+
...
+----------------------+----------------------+----------------------+-------------------------------------------+
| rNormal_reg          | Gaussian             | rnorm_reg_cpp.cpp    | rnorm_reg_cpp                             |
+----------------------+----------------------+----------------------+-------------------------------------------+
| rNormal_reg          | All non-Gaussian     | rnnorm_reg_cpp.cpp   | rnnorm_reg_cpp                            |
+----------------------+----------------------+----------------------+-------------------------------------------+
| rindependent         | Gaussian             | rindep_norm_gamma    | rindep_norm_gamma_reg_std_cpp             |
| _norm_gamma_reg      |                      | _reg_cpp.cpp         | rindep_norm_gamma_reg_std_parallel_cpp    |
+----------------------+----------------------+----------------------+-------------------------------------------+
...
- `rindependent_norm_gamma_reg` always uses C++ for the joint sampler.
...
rNormal_Gamma_reg           gaussian(identity)

rindependent_norm_gamma_reg gaussian(identity)
...
Function signature:
rNormal_Gamma_reg(n, y, x, prior_list,
                  offset = NULL, weights = 1,
                  family = gaussian(),
                  Gridtype = 2, n_envopt = NULL,
                  use_parallel = TRUE, use_opencl = FALSE,
...
Function signature:
rindependent_norm_gamma_reg(n, y, x, prior_list,
                            offset = NULL, weights = 1,
                            family = gaussian(),
                            Gridtype = 2, n_envopt = NULL,
                            use_parallel = TRUE, use_opencl = FALSE,
...
This file contains the four primary simfunctions (`rNormal_reg`, `rNormal_Gamma_reg`,
`rindependent_norm_gamma_reg`, `rGamma_reg`) and the R-level logic that interfaces with the
C/C++ backends.
...
- `rindep_norm_gamma_reg_std_parallel_cpp.cpp`  
