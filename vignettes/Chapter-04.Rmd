...
The 'glmbayes' package registers a `pp_check()` method for *glmb* fits (see `help(pp_check.glmb, package = "glmbayes")`), using 'bayesplot' on the same replicated responses produced by `simulate()`. For the menarche example (binomial proportions at a modest number of ages), histogram-style posterior predictive checks are often easier to read than density overlays, so we use `fun = "hist"` here.

```{r PPC_bayesplot_menarche, fig.height = 5, fig.width = 7}
