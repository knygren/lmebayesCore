...
```
all_src = OPENCL.cl
        + rmath (opencltools::load_kernel_library("rmath", package = "glmbayesCore"))
        + dpq   (opencltools::load_kernel_library("dpq", package = "glmbayesCore"))
        + nmath (opencltools::load_kernel_library("nmath", package = "glmbayesCore"))
        + kernel file (opencltools::load_kernel_source(...), e.g. "src/f2_f3_binomial_logit.cl")
```
...
Generic loaders live in **opencltools** (not re-exported from glmbayesCore):

- **`opencltools::load_kernel_source(relative_path, package = "glmbayesCore")`** – Loads a single `.cl` file from `inst/cl/`.
- **`opencltools::load_kernel_library(subdir, package = "glmbayesCore")`** – Loads all `.cl` files in a subdirectory (e.g., `"nmath"`, `"rmath"`, `"dpq"`), parses `@provides` and `@depends` annotations, performs a **dependency-aware topological sort**, and concatenates the files so that dependents appear after their dependencies.

The `@provides` and `@depends` tags allow modular, maintainable kernel code: each file declares what it provides and what it needs. See `?opencltools::load_kernel_library` for details.
...
Use `glmbayesCore_has_opencl()` to check availability and `diagnose_glmbayes()` for a full diagnostic report. See Chapter 16 for platform-specific installation instructions (Windows, Linux, macOS) and AMD ROCm notes.
...
- **Chapter 16** – GPU acceleration: installation, `glmbayesCore_has_opencl`, `diagnose_glmbayes`
