...
vignette: >
  %\VignetteIndexEntry{Chapter A5: t-statistics for Bayesian Linear models with normal-gamma priors}
...
-- In the conjugate parameterization above, posterior **v=2\alpha_{0}+n**. If the prior has positive **\alpha_{0}** (an informative prior on the variance), the posterior  **v** will be larger than the classical **n-p**. Increasing **v** makes the student-t closer to normal (lighter tails), so the tail probability at the same t value is smaller.
...
-- Bayesian marginal variance for **\beta_{j}** is **\frac{\beta_{n}}{\alpha_{n}} V_{n}[j,j]**.  Two effects happen here: (a) **V_{n}** generally skrinks relative to **(X^{TX})^{-1}** because of the prior precision **V_{0}^{-1}**, and (b) the posterior scale factor  **\frac{\beta_{n}}{\alpha_{n}}** can be smaller than the classical estimate **\hat(s)^{2}** depending on prior hyperparameters and the data (e.g., prior favors smaller variance). Either effect reduces the denominator and can increase the raw t value -but combined with larger **v** the net tail probability can still be smaller


--So you must compare both the numerator (posterior mean **m_{n,j}) vs OLS \hat{beta}_{j})  and the denominator (posterior scale vs **\hat{s}\sqrt{(X^{T}Xjj)^{-1}})**. Prior shrinkage often makes **mn,j** closer to zero and **V_{n}[j,j]** smaller, those reduce evidence against H0.

*** 1.3 Different centering (posterior mean vs OLS) and prior Shrinkage

- If the prior pulls **m_{n,j}** towards **\beta_{0}** (for example 0), the numerator of the Bayesian t is smaller in magnitude that **\beta_{ji}**, further reducing tail probability. In combination with (1) and (2), the posterior tail probability will often be noticably smaller than the classical one. Griffin & Brown and other skrinkage prior literature discuss this tradeoff explicitly for normal-gamma shrinkage priors.
...
\[
t_{\text{adj},i} = \frac{n}{n + n_{\text{prior}}} \cdot t_{\text{unadjusted},i} \cdot \sqrt{ \frac{n + n_{\text{prior}}}{n - p} }
\]
...
n+n_{prior}=n + n(pwt/(1-pwt)) =(n(1-pwt) +n (pwt))/(1-pwt)= n/(1-pwt)



The adjusted standard error is given by  
$ \text{Std\_err}_t = \text{Std\_err} \cdot \sqrt{ \frac{n - p}{n + n_{\text{prior}}} } $,  
which leads to the adjusted t-statistic:  
$ t_{\text{adj},i} = t_{\text{unadjusted},i} \cdot \sqrt{ \frac{n + n_{\text{prior}}}{n - p} } $.

$  Std_err_t = Std_err * \sqrt((n-p)/(n+n_prior))  $

which leads to 

$  t_adj_{i} = t_unadjusted_{i} * \sqrt(n+n_prior)/(n-p))  $
