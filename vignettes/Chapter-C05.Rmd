---
title: "Chapter C05: A Three-Term Total Variation Bound via Safe/Unsafe Decomposition"
author: "Kjell Nygren"
date: "`r Sys.Date()`"
output:
...
vignette: >
  %\VignetteIndexEntry{Chapter C05: A Three-Term Total Variation Bound via Safe/Unsafe Decomposition}
...
`vignette("Chapter-C03")` reproduces @Nygren2020's total variation bound for
...
This chapter develops the general-purpose device that stands in for it: a
...
available. It reproduces Sections 1–3 of the package's internal note
`inst/ELLIPSOID_TV_BOUND.md`, model-free: everything here concerns a
generic transition kernel \(P^n(x,\cdot)\), its target \(\pi(\cdot)\), and a
measurable partition of the state space. `Chapter-C06`/`Chapter-C07` and the
note's own §5–§8 supply the concrete \(A\), curvature floor and likelihood
that turn each general lemma below into a number.
...
zero extra terms. The bounds in this chapter are needed only once curvature
...
## A sharper split: converting the escape-mass term into an estimable quantity
...
see `inst/ELLIPSOID_TV_BOUND.md` §2.1 and §5–§8 for the argument in each
case. That refinement is outside the scope of this chapter, which stops at
...
estimable inequality this chapter is building toward.

# Bounding \(d_A\): a normalization-corrected version

## Why the naive approach is invalid
...
## Bounding the conditional term \(\|P^n_A - \pi_A\|_{TV}\)
...
to the model- and likelihood-specific chapters, since what the comparison
...
gives the headline result of this chapter.
...
3. **The correction term**, \(2[\pi(A^c) - P^n(x,A^c)]\), is the gap between
   ...
   `inst/ELLIPSOID_TV_BOUND.md`, and is left to that note's later sections.
...
# Connection to the package's convergence diagnostics
...
the calibrated `tv_tol`) in `inst/BLOCK_GIBBS_ERGODICITY_ING.md` §17. That
...
Sections 1–3 assemble Theorem 1 but deliberately leave two of its three
terms unfinished — bounded in the abstract, but not yet reduced to a
concrete, checkable condition. Both are open questions this chapter does
not resolve.
...
1. **When does the on-\(A\) gap inherit `Chapter-C03`'s bound?**
   ...
   multivariate normal densities on the *whole* space. Route 2 of §3.1
   ...
   *untruncated* comparison pair — with no extra slack — versus requiring an
   ...
   form \(q_j\), as in the ellipsoid case of `inst/ELLIPSOID_TV_BOUND.md`
   ...
   space. §6 below gives one partial route: it trades the need for a
...
2. **When does the correction term shrink quickly, and for which
   ...
   `inst/ELLIPSOID_TV_BOUND.md` Claim 5), but the resulting rate \(\rho\)
...
# A direct bound on the on-\(A\) gap via the escape-probability gap
...
§3.1 need machinery that does not yet exist for truncated, renormalized
...
## A computable heuristic: Corollary 1 in place of \(d_A(x)\)
...
`inst/ELLIPSOID_TV_BOUND.md` §6.1) rather than everywhere, the global
...
than comfortably inside it — the same caveat `inst/ELLIPSOID_TV_BOUND.md`
§6.1 attaches to treating \(M_j^{\text{pilot}}\) as a fixed threshold
rather than an estimated one.

# See also

- `vignette("Chapter-C03")` — @Nygren2020's total variation theory for the
  exactly-Gaussian case, which is the tool Theorem 1's on-\(A\) term
  reaches for once the safe region has a usable curvature sandwich.
- `vignette("Chapter-C04")` — the two-block Gibbs sampler this chapter's
  \(P^n(x,\cdot)\) refers to, in the fixed-precision case where
  \(A^c = \varnothing\) and every term but the on-\(A\) gap vanishes.
- `inst/ELLIPSOID_TV_BOUND.md` in the package sources — the full note,
  including the escape-probability bounds (§4), the drift/Lyapunov
  arguments that control \(P^n(x,A^c)\) explicitly, and the likelihood-
  specific specializations (§5–§8: multivariate-t, logit, probit, Poisson)
  that Sections 1–3 above deliberately factor out.
- `inst/BLOCK_GIBBS_ERGODICITY_ING.md` §17 — the simpler, already-
  implemented diagnostic this chapter's Theorem 1 refines.
- `inst/BLOCK_GIBBS_ERGODICITY.md`, `inst/P_MATRIX_MARGINAL_PRECISIONS.md`
  in the package sources — the spectral-radius and ellipsoid-score
  machinery Definition 1's safe region specializes to in the t-likelihood
  case.
