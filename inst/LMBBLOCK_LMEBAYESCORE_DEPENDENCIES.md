...
reached from `lmbBlock()`, directly or transitively. `normalize_group()`
...
│   └── lmebayesCore::normalize_group()  ★ ONLY lmebayesCore CALL
...
No node in this graph other than `normalize_group()` resolves to
...
### `lmebayesCore::normalize_group(block, l2)`

- **File:** [`R/simfunction_block_utils.R`](../R/simfunction_block_utils.R)
  (exported; `@export`, documented under `?simfuncs_group`).
...
- **Purpose:** normalizes the user-supplied `block` argument (factor,
  ...
  block). `lmbBlock()` uses only `group_info$k` and `group_info$rows` (via
...
|----------|------------------------|-------|
| `block_check_identifiability_xy(x, block, ...)` | Yes — `lmebayesCore::normalize_group()` (same single function, `lmebayes/R/lmbBlock.R:403`) | Standalone preflight diagnostic for block/hyper identifiability; users call it separately, `lmbBlock()` never calls it. |
...
`\code{\link[lmebayesCore]{rNormalGLM_reg_group}}` ("Gibbs conditional draws,
...
`lmebayesCore::rNormalGLM_reg_group()` (a full conditional draw inside a Gibbs
sweep, matrix API, used by `lmerb()`/`glmerb()`'s Block~1 — see
`R_EXPORT_REACHABILITY.md`). `lmbBlock()` **never calls**
`rNormalGLM_reg_group()` at runtime.
...
`lmebayesCore::normalize_group()`. No additional `lmebayesCore` surface.
...
compiled code — only the pure-R `normalize_group()` — **neither function is
...
- `grep -n "lmebayesCore" lmebayes/R/lmbBlock.R` → exactly 4 matches: 2 in
  ...
  (`lmebayesCore::normalize_group(...)`, once inside `.blmb_formula_block_meta()`
  and once inside `block_check_identifiability_xy()` — the latter not on
  `lmbBlock()`'s call graph).
- Read `lmebayesCore/R/simfunction_block_utils.R` (`normalize_group()`'s
...
| `normalize_group()` implementation | `R/simfunction_block_utils.R` |
