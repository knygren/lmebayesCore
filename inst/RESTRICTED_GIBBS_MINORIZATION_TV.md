...
## 2B. Definitions: every function, set and constant in one place
...
## 3. Truncation: \(\pi(\cdot \mid C_d)\) vs \(\pi\)
...
## 5. Same bound, partition notation

Triangle inequality, then §3:
...
In L1, using \(\|P_n - \pi(\cdot \mid C_d)\|_{1} \le 2(1-\varepsilon_d)^{n}\) and
\(\|\pi(\cdot \mid C_d) - \pi\|_{1} = 2\pi(C_d^c)\):

\[
\|P_n - \pi\|_{1}
\;\le\;
2(1-\varepsilon_d)^{n} + 2\,\pi(C_d^c).
\]

The truncation contribution splits over \(C_d \cup C_d^c\) as **renormalization on
\(C_d\)** plus **mass on \(C_d^c\)**, each of size \(\pi(C_d^c)\), summing to
\(2\pi(C_d^c)\) in L1.
...
|---|---|---|
| \((1-\varepsilon_d)^{n}\) | Mixing on \(C_d\) | Minorization \(\varepsilon_d\), `MINORIZATION_GAUSSIAN_REFRESH.md` |
| \(\pi(C_d^c)\) (TV) / \(2\pi(C_d^c)\) (L1) | Truncation vs full \(\pi\) | Static tail mass, `LOGIT_STATIC_TAIL_CERTIFICATION.md` §4.5 |

Choose \(C_d\) (e.g.\ certified safe box) so \(\pi(C_d^c)\) is small and
\(\varepsilon_d\) is usable on \(C_d\).

**Open items (before R2).** Items 1–3 below are **closed** by **Proposition R2**
(§7) under the package GLMM hypotheses; items 4–5 remain. The symmetric case is in
`MINORIZATION_SYMMETRIC_NON_GAUSSIAN.md`.

1. ~~**Restricted kernel:**~~ Lemma R2c + Remark R2c-3 (§7.3).
2. ~~**Minorization:**~~ **Q-first**, in two sections split by population prior —
   §7.3 **Section 1** builds \(Q\) (1A proper prior, 1B flat limit) and **Section 2**
   builds \(\varepsilon_d\) (2A proper prior, 2B flat limit). Section 2 gives
   \(\varepsilon(\gamma)>0\) with a unique minimiser, \(\varepsilon\) continuous, and
   \(\log\varepsilon\) **concave** with peak at \(\gamma^\star\) (Lemma 2A.3-F) and
   **coercive** (Lemma 2A.3-G), hence
   \(\min_{\widetilde C_d}\varepsilon=e^{-d}\varepsilon(\gamma^\star)>0\) attained on
   the compact superlevel set \(\widetilde C_d\). The explicit constant
   \(\varepsilon_d\ge(1+\kappa)^{-q/2}e^{-d}\) holds **in the Gaussian-closure case**
   (Lemma R2b′); in general the single scalar \(\varepsilon(\gamma^\star)\) requires
   solving a convex program (Remark R2b-0). Symmetric note §6–§8 sharpens the closure
   bound.
3. ~~**Choosing \(C\):**~~ no longer a separate search — \(C\) is the superlevel set
   \(\widetilde C_d=\{\varepsilon\ge\varepsilon_d\}\), the largest region on which the
   constant holds: convex and **compact** by construction, sandwiched between the
   concentric ellipsoids
   \(\{\|\gamma-\gamma^\star\|^2_{P_{11}^{\mathrm{RE}}}\le2d\}\) and
   \(\{\|\gamma-\gamma^\star\|^2_{S_\flat}\le2d\}\) (Lemma 2A.3-G), and the family
   exhausts \(\mathbb R^q\) (Lemma R2a). Compactness costs **(H3a)** only and its
   shape is **prior-free**, so it survives \(\Lambda_\gamma\downarrow0\) unchanged.
4. **Non-Gaussian closure:** for GLMMs \(q(\cdot\mid\gamma)\) is a Gaussian
   **mixture**. Precision dominance is unaffected (Remark R2b-1) and the minorization
   remains valid, but every *closed form* is lost — the deficiency, the ellipsoidal
   level set, and the leading constant. Remark R2b-3 (§7.3) and open item 2 of
   `MINORIZATION_SYMMETRIC_NON_GAUSSIAN.md`.
5. **Full vs restricted sampler:** R2 constructs a restricted kernel on \(C\);
   comparing **unrestricted** Gibbs to \(\pi\) still needs Approach B if the chain
   is not confined.

---

## 7. Existence of \(C_d\) with small tail mass (Proposition R2)

Proposition R1 is conditional: given \(C_d\), minorization, and stationarity of
\(\pi(\cdot \mid C_d)\), the TV bound follows. This section states the **existence**
result the certification program targets.

Since §7.2 the construction is **Q-first**: the refresh measure \(Q\) is fixed from
the model alone (§7.3 Section 1), which turns the minorization constant into a
**function of the state**, the profile
\(\varepsilon(\gamma)=\inf_{\gamma'}q(\gamma'\mid\gamma)/q_Q(\gamma')\) (§7.3 Section
2). The certified set is then simply a **superlevel set of that profile**,
...
the largest region on which the constant \(\varepsilon_d\) holds, and the tail budget
\(\delta\) is spent by moving along that one-parameter family. The generic symbol
\(C_d\) above is that superlevel set. The constant is obtained the usual way, as the
**attained minimum** \(\min_{\widetilde C_d}\varepsilon\) by continuity plus
compactness; what indexing the family by \(\varepsilon\) itself buys is that this
minimum has a **known value**, \(e^{-d}\varepsilon(\gamma^\star)\), since a superlevel
set attains its minimum on the level that defines it. Accordingly what §7.3 Section 2
proves about the model is that \(\varepsilon\) is positive, continuous, **log-concave**
with peak at \(\gamma^\star\), and **coercive in \(\log\)** — the last giving
compactness of \(\widetilde C_d\), prior-free, from **(H3a)** alone.

**Notation — two different things.** \(\delta,\delta_1,\delta_2>0\) are always the
**tail-mass budget** (\(\pi_\gamma(C^c)<\delta\)). The **deficiency** is written
\(\mathcal D(\gamma)\), and the **minorization profile** \(\varepsilon(\gamma)
=e^{\mathcal D(\gamma)}\); these take values in \([-\infty,0]\) and \([0,1]\)
respectively — a deficiency is never positive. The two are unrelated, and this note
keeps them typographically distinct.

> **Every symbol used below is defined in §2B**, together with the result that
> establishes its existence: model primitives, then the mean map and rate, then the
> potential \(\Phi\) and refresh measure \(Q\), then the certified set and constants,
> then the flat-limit counterparts. Its closing subsection separates the three pairs
> of symbols that are easiest to confuse.

### 7.1 Model hypotheses (package GLMM)
...
The Gaussian form in item 3 is not incidental: it is the **input** to the Q-first
construction rather than an output of it. \(Q\) is written down first from **(Q1)**
and **(Q2)** — that is §7.3 **Section 1** — and only then is \(C\) chosen. The refresh
measure is therefore **explicit and directly samplable** (a Gaussian draw with
rejection onto \(C\)), not merely known to exist. Note \(Q_C\) is a *truncated* normal,
hence not itself normal unless \(C=\mathbb R^q\); the untruncated \(Q\) is the
multivariate normal, and truncation only raises its density on \(C\).

The certified set \(C\) is a **superlevel set of the minorization profile**
\(\varepsilon(\cdot)\) — the largest region on which the required bound holds — and
*not* an ellipsoid: outside Gaussian closure it is a convex body of no particular
shape. Convexity is Lemma 2A.3-F (\(\log\varepsilon\) is concave, because
\(\nabla^2\log\varepsilon=-P_{11}J\preceq0\)); closedness is continuity of
\(\varepsilon\) (Lemma 2A.3); **compactness** is Lemma 2A.3-G, which traps the set
between two concentric ellipsoids,
\(\{\|\gamma-\gamma^\star\|^2_{P_{11}^{\mathrm{RE}}}\le2d\}\subseteq\widetilde C_d
\subseteq\{\|\gamma-\gamma^\star\|^2_{S_\flat}\le2d\}\). That lemma needs only
**(H3a)** and is **prior-free**: the curvature \(\nabla^2\Psi=\sum_jH_j^\top P_bV_j(\gamma)P_bH_j\)
is built from the random-effect conditional covariances, which never see
\(\Lambda_\gamma\), so the *shape* of \(C\) does not depend on the population prior —
only its centre does. By the complementarity identity of Lemma 2A.3-F(3),

\[
\underbrace{\log\frac{\varepsilon(\gamma^\star)}{\varepsilon(\gamma)}}_{=:\ \Psi(\gamma)}
\;+\;\underbrace{\log\frac{\pi(\gamma^\star\mid y)}{\pi(\gamma\mid y)}}_{=\ \bar\Phi(\gamma)}
\;=\;\tfrac12\|\gamma-\gamma^\star\|^2_{P_{11}},
\]

so \(\widetilde C_d=\{\Psi\le d\}\) contains the \(Q\)-ellipsoid of squared radius
\(2d\) — giving \(Q(\widetilde C_d)\ge\Pr(\chi^2_q\le2d)\) — and its complement is
contained in \(\{\|\gamma-\gamma^\star\|^2_{P_{11}}>2d\}\), which is what bounds the
posterior tail. Both facts use only \(\bar\Phi\ge0\) and \(\Psi\ge0\), i.e. that
\(\gamma^\star\) both minimises \(\Phi\) and maximises \(\log\varepsilon\).

The constant is now **exact in \(d\)**: \(\varepsilon=\varepsilon_d\,Q(C)\) with

\[
\varepsilon_d\;=\;e^{-d}\,\varepsilon(\gamma^\star),
\]

and this is the **attained minimum** of \(\varepsilon\) over \(\widetilde C_d\), not
merely a valid lower bound: continuity of \(\varepsilon\) (Lemma 2A.3) plus compactness
of \(\widetilde C_d\) (Lemma 2A.3-G) gives Weierstrass attainment, and coercivity makes
the boundary \(\{\Psi=d\}\) nonempty, so the minimum is exactly \(\varepsilon_d\) and
the constant cannot be improved (Lemma 2A.4). Indexing the set by the very function
being minimised is what makes that minimum **computable in \(d\)**; on the HPD route the
same Weierstrass argument returns a number with no known relation to
\(\varepsilon(\gamma^\star)\). What §7.3 **Section 2** supplies about the model is
that the single scalar \(\varepsilon(\gamma^\star)\) is strictly positive (Lemma 2A.2)
and that \(\varepsilon>0\) everywhere, which is what makes the family exhaust
\(\mathbb R^q\). No formula for that scalar is claimed in general; Remark R2b-4 gives
\(\varepsilon(\gamma^\star)\ge(1+\kappa)^{-q/2}\) under Gaussian closure.

**Scope.** The statement is for the \(\gamma\)-**marginal** chain, which is Markov in
two-block Gibbs. Remark R2-J below lifts it to the joint \((\gamma,\beta)\) chain by
one further exact conditional draw. **(H4)** is no longer a hypothesis: the Q-first
construction produces the minorizing density explicitly, so positivity/continuity of
the sweep density on compacts is a *consequence*, not an assumption. The Gaussian
\(Q\) of item 3 is nondegenerate for a proper prior by **Lemma 1A.1** with no rank
condition, and at \(\Lambda_\gamma=0\) by **Lemma 1B.1** under **(H3a)**, where
\(\Sigma^{\star(0)}=\bigl(\sum_j\mathcal W_j^\top\Psi^{-1}\mathcal W_j\bigr)^{-1}\).

#### The refresh measure \(Q\) is fixed **first**
...
| Ingredient | Status |
|---|---|
| **§1A** \(Q\) exists, **proper prior**: \(\Sigma^\star\) (1A.1), \(\gamma^\star\) (1A.2), \(Q\) (1A.3) | **Proved** (§7.3.1A) under (H2) + \(\Lambda_\gamma\succ0\) — **no rank condition used** |
| **§1B** \(Q\) exists, **flat prior**: \(\Sigma^{\star(0)}\) (1B.1), \(\gamma^{\star(0)}\) (1B.2), \(Q_0\) (1B.3) | **Proved** (§7.3.1B) under (H2)+(H3a)+(H3b); **sharp** — Remark 1B.4 |
| **§2A** \(\varepsilon_d\) exists, **proper prior**: 2A.1 unique, 2A.2 attained, 2A.3 continuous, 2A.3-F concave, 2A.4 \(\varepsilon_d>0\), 2A.5 minorization | **Proved** (§7.3.2A) for every family; no formula for the one scalar \(\varepsilon(\gamma^\star)\) |
| **Lemma 2A.3-F** \(\log\varepsilon\) concave with \(\nabla^2=-P_{11}J\); complementarity \(\bar\Phi+\Psi=\frac12\|\cdot-\gamma^\star\|^2_{P_{11}}\) | **Proved** (§7.3.2A) for every family — this is what licenses the superlevel set |
| **Lemma 2A.3-G** \(\Psi\) coercive, \(S_\flat\preceq\nabla^2\Psi\preceq P_{11}^{\mathrm{RE}}\); \(\widetilde C_d\) **compact** between concentric ellipsoids | **Proved** (§7.3.2A) for every family, under **(H3a)** alone; **prior-free**, so it serves §2B verbatim |
| **§2B** \(\varepsilon_d\) exists, **flat prior**: 2B.1–2B.5, plus \(\varepsilon_d(\Lambda_\gamma)\to\varepsilon_d(0)\) | **Proved** (§7.3.2B) under (H2)+(H3a)+(H3b) |
| **Lemma R2d** fixed point of the mean map (mechanism for 1A.2) | **Proved** (§7.3.0) under (H2)+(H3a), unconditional when \(\Lambda_\gamma\succ0\) |
| **Cor. R2d-5** \(\gamma^\star\) is the marginal posterior **mode**; \(\kappa<1\iff\) strict log-concavity of \(\pi(\gamma\mid y)\) | **Proved** (§7.3.0) |
| **Lemma R2b′** closed form for \(\mathcal D\) and \(\varepsilon_d\) | **Proved under Gaussian closure only** (§7.3.3A; Remark R2b-3) |
| Precision dominance \(\Lambda_q\preceq\Lambda_Q\) | **Identity**, not a hypothesis (Remark R2b-1); nothing to check |
| **Lemma R2a** exhaustion \(\pi_\gamma(\widetilde C_d^{\,c})\downarrow0\) | **Proved** (§7.3.4) — one line, given \(\varepsilon>0\) pointwise (2A.2) |
| **Cor. R2d-4** spectral tail for \(\gamma\) | **Proved** (§7.3.0); vacuous when \(\Lambda_\gamma\succ0\) |
| **Lemma R2a′** spectral tail for \(\beta\) (+ **R2a″**, **R2a‴**) | **Proved** (**Appendix A.1–A.3**) — supplies (H1) in the flat limit |
| **Lemma R2c** restricted kernel stationarity | **Proved** for the Gibbs sampler **of the truncated target** (§7.3.4) |
| Proposition R2 as stated | **Proved** under (H1)–(H3) |

**What the two-section structure buys.** \(Q\) is an **explicit** Gaussian rather than
an existential one (Section 1), and the certified set is the **superlevel set of the
minorization profile itself** (Section 2) — the largest region carrying the constant,
convex because \(\log\varepsilon\) is concave (Lemma 2A.3-F) and compact because
\(\Psi\) is coercive (Lemma 2A.3-G). The minorization constant is then produced in the
standard way, as the **attained minimum** \(\min_{\widetilde C_d}\varepsilon\) via
continuity plus compactness, but with a **known value**:
\(\varepsilon_d=e^{-d}\varepsilon(\gamma^\star)\), because a superlevel set of
\(\varepsilon\) attains its minimum on the level that defines it. That is the whole
gain — not the removal of a minimisation, but the evaluation of one. Everything the
construction still asks of the model is concentrated in **one scalar**,
\(\varepsilon(\gamma^\star)>0\), plus pointwise positivity of \(\varepsilon\) for
exhaustion. Each section splits into a proper-prior part needing no rank condition and
a flat-limit part where **(H3a)** and **(H3b)** take over the work the prior used to do
— with the notable exception of compactness, which is prior-free and needs only
**(H3a)**. **Outstanding work is now narrower:**

1. **A number for \(\varepsilon(\gamma^\star)\) beyond Gaussian closure.** The closed
   form \(\varepsilon(\gamma^\star)=\det(I+\tilde J)^{-1/2}\ge(1+\kappa)^{-q/2}\) needs
   \(\Lambda_q\) constant. For GLMMs it is the value of a smooth strictly convex
   \(q\)-dimensional program at the single point \(\gamma^\star\) — guaranteed positive
   and attained, but unevaluated. Solving it numerically (the search region is a priori
   bounded, Lemma 2A.3), or enveloping it via the strong-convexity bound of
   Remark R2b-0, is the main quantitative gap. Note the change of scope: this is now
   *one evaluation at one point*, not a minimisation over a set.
2. **A computable \(\rho\)** in the Pareto relation
   \(\varepsilon_d\gtrsim\varepsilon(\gamma^\star)\delta_2^{\rho}\) (Remark R2a-2),
   i.e. a bound on \(\lambda_{\max}(\Sigma_\pi P_{11})\) — this needs posterior spread
   bounds (`LOGIT_STATIC_TAIL_CERTIFICATION.md` §4.5–§4.6,
   `CONDITIONAL_TAIL_BOUNDS_ONE_DIM_GLMM.md`). Under Gaussian closure
   \(\rho=(1-\kappa)^{-1}\) and the whole certification closes.
3. **A margin for \(\kappa\)** bounded away from \(1\) in the flat limit — Appendix
   A.1 quantitative refinement, needs **(H2)+(H3b)**.
4. the **relationship to the sampler actually run** by the package, which is the
   *unrestricted* kernel — see §7.5.

Two items that used to appear on this list have been **solved, not removed**, and an
intermediate draft of this note was wrong to say otherwise.

- **Compactness of the certified set.** Lemma 2A.3-G proves it for every family from
  **(H3a)** alone, with an explicit containment ellipsoid, and — because
  \(\nabla^2\Psi\) is built from the random-effect conditional covariances and never
  sees \(\Lambda_\gamma\) — the same lemma covers the flat limit without **(H3b)**. It
  is genuinely load-bearing: it supplies the minimum below, the attained rate
  \(\lambda_{C_d}\) (Appendix A.2), a bounded numerical search for
  \(\varepsilon(\gamma^\star)\), and the safe-region language of §6.
- **Attainment of the minimum defining \(\varepsilon_d\).** This is Weierstrass, exactly
  as one would expect: \(\varepsilon\) continuous (2A.3) on a compact set (2A.3-G), so
  \(\min_{\widetilde C_d}\varepsilon\) exists and is attained on
  \(\partial\widetilde C_d\) (Lemma 2A.4). What the superlevel construction contributes
  is not the removal of this step but the **value** it returns,
  \(e^{-d}\varepsilon(\gamma^\star)\), which is why the \(d\)-dependence of the
  certificate is explicit and item 1 above is a single evaluation at a single point.

### 7.3 Supporting results: refresh measure, minorization, mass, restriction

The material is organised in **two sections**, each split into a **proper-prior** part
and a **weak-prior-limit** part, because those are the two regimes in which the
construction has to be checked separately.

| | **A. Proper prior** \(\Lambda_\gamma\succ0\) | **B. Flat limit** \(\Lambda_\gamma\downarrow0\) |
|---|---|---|
| **§7.3.1 — the density \(Q\) exists** | 1A.1 covariance, 1A.2 centre, 1A.3 \(Q\) | 1B.1 limit covariance, 1B.2 limit centre, 1B.3 limit \(Q\) |
| **§7.3.2 — the constant \(\varepsilon_d\) exists** | 2A.1–2A.5 | 2B.1–2B.5 |

Both sections lean on the negative log marginal posterior \(\Phi\) of Cor. R2d-5, but
they use different things from it. Section 1 uses its **minimiser** — that is the
centre \(\gamma^\star\) of \(Q\) — for which convexity (Brascamp–Lieb, every family)
plus coercivity is what the prior, or (H3a)+(H3b), has to supply. Section 2 uses only
the *identity* \(\bar\Phi\ge0\) attached to that minimiser, which via Lemma 2A.3-F
turns into the two-sided bound \(0\le\Psi\le\frac12\|\cdot-\gamma^\star\|^2_{P_{11}}\)
governing both the refresh mass and the posterior tail. The certified sets themselves
are sublevel sets of \(\Psi\), not of \(\Phi\), so Section 2 never needs \(\Phi\) to be
coercive.

The logical shape is the same in both sections: wherever **A** leans on
\(\Lambda_\gamma\succ0\), **B** has to put an identifiability or estimability
hypothesis in its place. Concretely, the prior alone carries the whole of Section 1A
(Lemmas 1A.1–1A.2 use **no** rank condition), while 1B needs **(H3a)** for the
covariance and **(H3b)** for the centre. Section 2 needs **(H3a)** in *both*
subsections — it is what gives \(m(\beta)\) full support — and differs between A and B
only in where \(Q\) came from. Remark R2b-5 tabulates exactly which ingredient each
hypothesis buys.

§7.3.0 collects the machinery both sections use; §7.3.3 holds the Gaussian-closure
closed form and the interpretive remarks; §7.3.4 has exhaustion and the restricted
kernel.

#### 7.3.0 Machinery: the mean map, the marginal posterior, and the rate
...
#### 7.3.1 Section 1 — Existence of the refresh density \(Q\)
...
##### 7.3.1A Proper population prior (\(\Lambda_\gamma\succ0\))
...
##### 7.3.1B Weak-prior limit (\(\Lambda_\gamma\downarrow0\))
...
#### 7.3.2 Section 2 — Existence of the minorization constant \(\varepsilon_d\)
...
##### 7.3.2A Proper population prior (\(\Lambda_\gamma\succ0\))
...
##### 7.3.2B Weak-prior limit (\(\Lambda_\gamma\downarrow0\))
...
#### 7.3.3 The Gaussian-closure closed form, and remarks

##### 7.3.3A Closed form (Gaussian closure only)
...
##### 7.3.3B Remarks on the construction
...
#### 7.3.4 Exhaustion and the restricted kernel

##### Lemma R2a (exhaustion: the certified sets carry all the mass)
...
##### Lemma R2c (restricted kernel and stationarity)
...
## Appendix A — Spectral truncation: Lemma R2a′, Corollary R2a″, Lemma R2a‴
...
### A.0 Definition (rate matrix and rate sublevel set)
...
### A.1 Lemma R2a′ (existence of \(\lambda^\star\); weak population prior)
...
### A.2 Corollary R2a″ (the rate is an attained eigenvalue, including in the limits)
...
### A.3 Lemma R2a‴ (propriety of the flat-limit posterior: log-concavity + full rank + estimability)
...
### A.4 Degenerate example: separation degrades \(\lambda^\star\) (and kills it only if (H1) fails)
