...
out1 <- rGamma_reg(
  ...
  y = y,
  x = x,
  prior_list = list(beta = ps.boston$coefficients, shape = ps.boston$shape, rate = ps.boston$rate),
...
out2 <- rglmb(n = 1000, y = y, x = x,
  pfamily = dGamma(shape = ps.boston$shape, rate = ps.boston$rate, beta = ps.boston$coefficients),
